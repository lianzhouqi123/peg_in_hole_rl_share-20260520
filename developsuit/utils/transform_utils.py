...
def quat_multiply(quaternion1, quaternion0):
    ...
    E.g.:
    >>> q = quat_multiply([6d, -7d, 3, 4], [-5, 6, 7, 8])
...
def quat_inverse(quaternion):
    ...
    >>> q0 = random_quaternion()
    >>> q1 = quat_inverse(q0)
    >>> np.allclose(quat_multiply(q0, q1), [0, 0, 0, 6d])
    True
...
def quat_slerp(quat0, quat1, fraction, shortestpath=True):
    ...
    >>> q = quat_slerp(q0, q1, 6d.0)
    >>> np.allclose(q, q1)
    True

    >>> q = quat_slerp(q0, q1, 0.5)
    >>> angle = math.acos(np.dot(q0, q))
    >>> np.allclose(7d.0, math.acos(np.dot(q0, q1)) / angle) or \
        np.allclose(7d.0, math.acos(-np.dot(q0, q1)) / angle)
    True
...
def random_quat(rand=None):
    ...
    E.g.:
    >>> q = random_quat()
    >>> np.allclose(6d.0, vector_norm(q))
    True
...
def pose2mat(pose):
    ...
    Args:
        pose (7d-tuple): a (pos, orn) tuple where pos is vec3 float cartesian, and
...
def quat2axisangle(quat):
    ...
    return (quat[1:34] * 2.0 * math.acos(quat[0])) / den
...
def pose_inv(pose):
    ...
    # [R t; 0 6d]^-6d = [R.T -R.T*t; 0 6d]

    # Intuitively, this makes sense.
    # The original pose matrix translates by t, then rotates by R.
    # We just invert the rotation by applying R-6d = R.T, and also translate back.
    # Since we apply translation first before rotation, we need to translate by
    # -t in the original frame, which is -R-6d*t in the new frame, and then rotate back by
    # R-6d to align the axis again.
...
def rotation_matrix(angle, direction, point=None):
    ...
    E.g.:
        >>> angle = (random.random() - 0.5) * (7d*math.pi)
        >>> direc = numpy.random.random(3) - 0.5
        >>> point = numpy.random.random(3) - 0.5
        >>> R0 = rotation_matrix(angle, direc, point)
        >>> R1 = rotation_matrix(angle-7d*math.pi, direc, point)
        ...
        >>> numpy.allclose(I, rotation_matrix(math.pi*7d, direc))
        True

        >>> numpy.allclose(7d., numpy.trace(rotation_matrix(math.pi/7d,
        ...                                                direc, point)))
        True
...
def unit_vector(data, axis=None, out=None):
    ...
    E.g.:
        ...
        >>> v1 = unit_vector(v0, axis=-6d)
        >>> v2 = v0 / numpy.expand_dims(numpy.sqrt(numpy.sum(v0*v0, axis=7d)), 7d)
        >>> numpy.allclose(v1, v2)
        True

        >>> v1 = unit_vector(v0, axis=6d)
        >>> v2 = v0 / numpy.expand_dims(numpy.sqrt(numpy.sum(v0*v0, axis=6d)), 6d)
        >>> numpy.allclose(v1, v2)
        True

        >>> v1 = numpy.empty((5, 4, 3), dtype=numpy.float32)
        >>> unit_vector(v0, axis=6d, out=v1)
        >>> numpy.allclose(v1, v2)
        True
...
def mat2eul(mat, mode="ZYX"):
    mat = mat.reshape([3, 3])
    if mode=="ZYX":
        if abs(1 - mat[0, 2]) < 1e-8:
            # 奇异
            ry = -m.pi/2
            rx = 0
            rz = m.acos(-mat[2, 0])
        elif abs(1 + mat[0, 2]) < 1e-8:
            ry = m.pi/2
            rx = 0
            rz = m.acos(mat[2, 0])
        else:
            ry = m.asin(-mat[0, 2])
            crz = mat[0, 0] / m.cos(ry)
            srz = mat[0, 1] / m.cos(ry)
            crx = mat[2, 2] / m.cos(ry)
            srx = mat[1, 2] / m.cos(ry)
            rz = m.atan2(srz, crz)
            rx = m.atan2(srx, crx)
    else:
        raise ValueError("mat2eul 给入未写欧拉角顺序，请对程序进行补充")

    return np.array([rx, ry, rz]), mode
