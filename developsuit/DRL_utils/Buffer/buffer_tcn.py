...
class PPOBufferWarp:
    def __init__(self, num_steps, num_envs, obs_shape, critic_obs_shape, action_dim):
        """
        全张量化多环境并行 Buffer。
        🌟 升级：支持任意维度的 obs_shape，例如一维的 (40,) 或是时序的 (5, 8)
        """
        self.num_steps = num_steps
        self.num_envs = num_envs
        
        # 兼容处理：如果传入的是整数 40，转为 (40,)；如果是元组/列表 (5, 8)，直接使用
        self.obs_shape = (obs_shape,) if isinstance(obs_shape, int) else tuple(obs_shape)
        self.critic_obs_shape = (critic_obs_shape,) if isinstance(critic_obs_shape, int) else tuple(critic_obs_shape)
        
        self.action_dim = action_dim
        self.step = 0

        # ========== 预分配 3D/4D GPU 内存 ==========
        # 利用 *self.obs_shape 解包，动态支持 (num_steps, num_envs, 5, 8)
        self.obs = torch.zeros((num_steps, num_envs, *self.obs_shape), dtype=torch.float32, device=device)
        self.critic_obs = torch.zeros((num_steps, num_envs, *self.critic_obs_shape), dtype=torch.float32, device=device)
    ...
    def mpl_generator(self, num_mini_batch, mini_batch_size=None):
        ...
        # 仅展平原始存储，按 mini-batch 懒索引，避免每个 epoch 先复制整份打乱后的大张量
        flat_obs = self.obs[:self.step].view(-1, *self.obs_shape)
        flat_critic_obs = self.critic_obs[:self.step].view(-1, *self.critic_obs_shape)
        flat_actions = self.actions[:self.step].view(-1, self.action_dim)
        flat_returns = self.returns[:self.step].view(-1)
        flat_log_probs = self.log_probs[:self.step].view(-1)
        flat_zs = self.zs[:self.step].view(-1, self.action_dim)
        flat_advantages = self.advantages[:self.step].view(-1)

        for i in range(num_mini_batch):
            start = i * mini_batch_size
            end = start + mini_batch_size
            batch_idx = sampler[start:end]
